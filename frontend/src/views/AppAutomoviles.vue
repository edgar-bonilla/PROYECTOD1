          ...
          <div class="card-body">
            ...
            <form @submit.prevent="createAutomovil" class="needs-validation" novalidate>
              ...
              <div class="mb-3 row">
                ...
                <div class="col-sm-7">
                  <input type="text" v-model="nuevoAutomovil.velocidad_maxima" id="velocidad_maxima" class="form-control"
                    placeholder="Ingrese la velocidad máxima" required />
                </div>
              </div>
              <div class="mb-3 row">
          ...
          <div class="card-body">
            ...
            <form @submit.prevent="updateAutomovil" class="needs-validation" novalidate>
              ...
              <div class="mb-3 row">
                <label for="velocidad_maxima" class="col-sm-2 col-form-label">Velocidad Máxima</label>
                <div class="col-sm-7">
                  <input type="text" v-model="automovilEditado.velocidad_maxima" id="velocidad_maxima" class="form-control"
                    placeholder="Ingrese la velocidad máxima" required />
                </div>
              </div>
              <div class="mb-3 row">
...
export default {
  ...
  async created() {
    try {
      const automovilesResponse = await axios.get('http://localhost:3000/api/automoviles');
      this.automoviles = automovilesResponse.data;

      const fabricantesResponse = await axios.get('http://localhost:3000/api/fabricantes');
      this.fabricantes = fabricantesResponse.data;

      const disenadoresResponse = await axios.get('http://localhost:3000/api/disenadores');
      this.disenadores = disenadoresResponse.data;
    } catch (error) {
  ...
  methods: {
    ...
    cancelCreate() {
      this.showTab = 'table';
      this.nuevoAutomovil = { nombre: '', año: '', velocidad_maxima: '', fabricante_id: '', disenador_id: '' };
    },
    async createAutomovil() {
      try {
        const response = await axios.post('http://localhost:3000/api/automoviles', this.nuevoAutomovil);
        this.automoviles.push(response.data);
        this.cancelCreate();
      } catch (error) {
        console.error('Error creando el automóvil:', error);
      }
    },
    async editAutomovil(automovil) {
      this.automovilEditado = { ...automovil };
      this.showTab = 'edit';
    },
    cancelEdit() {
      this.showTab = 'table';
      this.automovilEditado = null;
    },
    async updateAutomovil() {
      try {
        const response = await axios.put(`http://localhost:3000/api/automoviles/${this.automovilEditado.id}`, this.automovilEditado);
    ...
    async deleteAutomovil(automovil) {
      try {
        await axios.delete(`http://localhost:3000/api/automoviles/${automovil.id}`);
